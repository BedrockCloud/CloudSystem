...
final class Utils {
    ...
    public static function cleanPath(string $path, bool $removePath = false): string {
        ...
        return str_replace(CLOUD_PATH, rtrim(str_replace("bedrockcloud", "pcsrc", basename(CLOUD_PATH)), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR, $path);
    }

    public static function check(): void {
        if (Utils::checkRunning($pid)) {
            CloudLogger::get()->error("Another instance of §6Bedrock§eCloud §ris already running! (ProcessId: " . $pid . ")");
            exit(1);
        }

        if (PHP_OS_FAMILY == "Windows") {
            CloudLogger::get()->error("You can't use §6Bedrock§eCloud §ron Windows!");
            exit(1);
        }
    ...
    public static function downloadFiles(): void {
        ...
        if ($downloadServerPlugin) {
            ...
            Utils::download("https://github.com/BedrockCloud/CloudBridge/releases/latest/download/CloudBridge.phar", $serverPluginsPath . "CloudBridge.phar");
        ...
        if ($downloadProxyPlugin) {
            ...
            Utils::download("https://github.com/BedrockCloud/CloudBridge-Proxy/releases/latest/download/CloudBridge.jar", $proxyPluginsPath . "CloudBridge.jar");
